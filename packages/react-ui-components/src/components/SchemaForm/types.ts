...
export interface SchemaFormProps<T, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any> extends Partial<FormProps<T, S, F>> {
	...
	/**
	 * If set to `true`, it will not display the switch that allows users to toggle the visibility of all error messages.
	 * Default: `false`, meaning that the switch will be displayed.
	 */
	hideErrorsSwitch?: boolean;
}
