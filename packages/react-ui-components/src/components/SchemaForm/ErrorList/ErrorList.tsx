...
const ErrorList = ({ errors }: ErrorListProps) => {
	const errorsHash: { [prop: string]: string[] } = errors.reduce((acc: { [prop: string]: string[] }, error: AjvError) => {
		if (isEmpty(acc[error.property])) {
			acc[error.property] = [`${error.property} ${error.message}`];
		} else {
			acc[error.property] = [...acc[error.property], `${error.property} ${error.message}`];
		}
		return acc;
	}, {});
