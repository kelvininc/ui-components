...
import { ISelectSingleOption, selectHelper } from '@kelvininc/ui-components';
...
const ADD_SELECT_OPTION: ISelectSingleOption = {
	label: 'Add a new Tag',
	icon: EIconName.Add,
	value: 'add-option',
	selectable: true,
...
const AddOptionTemplate: ComponentStory<typeof KvSingleSelectDropdown> = args => {
	...
	const optionsWithAddOption = useMemo(() => {
		const newOptions = { ...options };
		newOptions[ADD_SELECT_OPTION.value] = ADD_SELECT_OPTION;
		return newOptions;
	}, [options]);

	const filteredOptionsWithAddOption = useMemo(() => {
		const newFilteredOptions = { ...filteredOptions };
		newFilteredOptions[ADD_SELECT_OPTION.value] = ADD_SELECT_OPTION;
		return newFilteredOptions;
	}, [filteredOptions]);

	const setSearchTerm = (newSearchTerm: string): void => {
		updateArgs({ searchValue: newSearchTerm });
	};

	const selectOption = (selectedOption: string): void => {
		if (selectedOption !== ADD_SELECT_OPTION.value) {
			updateArgs({ selectedOption });
		}
	};
	...
	return (
		<KvSingleSelectDropdown
			{...args}
			options={optionsWithAddOption}
			filteredOptions={filteredOptionsWithAddOption}
			onSearchChange={({ detail: newSearchTerm }) => setSearchTerm(newSearchTerm)}
			onOptionSelected={({ detail: newOption }) => selectOption(newOption)}
		/>
	);
};
