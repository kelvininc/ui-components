...
import { createFormattedDateFromTimestampInTimezone, createTimestampInTimezoneFromFormattedDate } from '../time-picker/time-picker.helper';
...
export const getFormattedSelectedDates = (range: SelectedTimestamp, mode: EAbsoluteTimePickerMode, timezoneName: string): SelectedRange => {
	...
	if (mode === EAbsoluteTimePickerMode.Single) {
		...
		return [createFormattedDateFromTimestampInTimezone(date, timezoneName, CALENDAR_DATE_TIME_MASK)];
	}

	const [from, to] = range;
	const dateFrom = createFormattedDateFromTimestampInTimezone(from, timezoneName, CALENDAR_DATE_TIME_MASK);
	const dateTo = !isNil(to) ? createFormattedDateFromTimestampInTimezone(to, timezoneName, CALENDAR_DATE_TIME_MASK) : undefined;
