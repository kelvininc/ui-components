...
export const buildFooterConfig = (steps?: IWizardStep[], currentStep?: number, currentStepState?: EStepState, disabled = false): IWizardFooter => {
	if (isEmpty(steps) || isNil(currentStep) || currentStep < 0 || currentStep > steps.length - 1) {
		return null;
	}
	...
	const stepsConfig: IStepBarStep[] = steps.map((step, index) => {
		return {
			stepKey: step.title,
			enabled: !disabled && ((index === currentStep + 1 && currentStepState === EStepState.Success) || (index < currentStep && steps[index + 1].allowGoBack)),
	...
	return {
		...
		nextEnabled: !disabled && currentStepState === EStepState.Success,
		showCancelBtn: steps[currentStep].cancelable ?? false,
		cancelEnabled: true,
		showCompleteBtn: currentStep === steps.length - 1,
		completeEnabled: !disabled && currentStepState === EStepState.Success,
