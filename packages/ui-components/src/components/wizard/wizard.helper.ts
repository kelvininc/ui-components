...
export const buildHeaderConfig = (steps?: IWizardStep[], currentStep?: number): IWizardHeader | null => {
	if (steps === undefined || steps.length === 0 || currentStep === undefined || currentStep === null || currentStep < 0 || currentStep > steps.length - 1) {
		return null;
	}
...
export const buildFooterConfig = (steps?: IWizardStep[], currentStep?: number, currentStepState?: StepState, disabled = false): IWizardFooter | null => {
	if (steps === undefined || steps.length === 0 || currentStep === undefined || currentStep === null || currentStep < 0 || currentStep > steps.length - 1) {
		return null;
	}
