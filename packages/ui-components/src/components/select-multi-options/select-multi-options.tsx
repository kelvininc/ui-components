...
import { ADD_OPTION, DEFAULT_NO_DATA_AVAILABLE_LABEL, MINIMUM_SEARCHABLE_OPTIONS, SELECT_OPTION_HEIGHT_IN_PX } from './select-multi-options.config';
...
export class KvSelectMultiOptions implements ISelectMultiOptionsConfig, ISelectMultiOptionsEvents {
	...
	@Listen('create')
	createOptionHandler(event: CustomEvent<string>) {
		this.onCreateNewOption(event.detail);
	}
	@Listen('cancel')
	...
	buildSelectionOptions() {
		const selectOptions = buildSelectOptions(this.options, this.options, this.selectedOptions, this.highlightedOption, this.canAddItems);
		const selectSelectableOptions = getSelectableOptions(this.options);
		const selectCurrentOptions = buildSelectOptions(this.currentOptions, this.options, this.selectedOptions, this.highlightedOption, this.canAddItems);
	...
	private onCreateNewOption = (newOption: string): void => {
		this.optionCreated.emit(newOption);
		this.isCreating = false;
	};

	private onCancelNewOption = (): void => {
		this.isCreating = false;
	};
					...
					>
						<div class="create-new-option-form">
							<slot name="create-new-option">
								<kv-select-create-option onCreate={({ detail: newOption }) => this.onCreateNewOption(newOption)} onCancel={this.onCancelNewOption} />
							</slot>
						</div>
					</div>
				)}
