			...
			>
				...
				{hasNoDataAvailable && (
					<slot name="no-data-available">
						<kv-illustration-message class="empty-state" {...this.noDataAvailableConfig} />
					</slot>
				)}
				{hasNoResultsFound && (
					<slot name="no-results-found">
						<div class="no-results-found">
							<kv-illustration-message class="empty-state" {...this.noResultsFoundConfig} />
							{this.canAddItems && (
								<div class="create-new-option-button">
									<kv-select-option
										{...buildNewOption(this.highlightedOption, this.createInputPlaceholder)}
										onItemSelected={this.onItemSelected}
										style={{
											'--select-option-height': `${SELECT_OPTION_HEIGHT_IN_PX}px`
										}}
